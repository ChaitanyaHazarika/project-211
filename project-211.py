    ...
    if len(all_vehicles) > 1:
        ...
        for distance_in_meter in final_vehicle_result:
            if distance_in_meter['distance'] >10 and distance_in_meter['distance'] < 15:
                ...
                dropped_vehicle.apply_control(carla.VehicleControl(throttle = 1 , steer= -1))
                time.sleep(2)

                dropped_vehicle.apply_control(carla.VehicleControl(throttle= 1, steer= 1))
                time.sleep(3)

                dropped_vehicle.apply_control(carla.VehicleControl(throttle= 0.999999999999, ))
                time.sleep(3)

                print("hello close car name: ", distance_in_meter['vehicle_name'])

            if distance_in_meter['distance'] > 3 and distance_in_meter['distance'] < 6:
                dropped_vehicle.apply_control(carla.VehicleControl(throttle= 1, hand_brake = True))
                time.sleep(5)
